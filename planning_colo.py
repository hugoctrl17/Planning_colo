...
taches_input = st.text_area(
    "Une tâche par ligne",
    value=(
        "NETTOYAGE GITE MATIN\n"
        "PREPA PIQUE NIQUE\n"
        "PREPA REPAS MIDI\n"
        "NETTOYAGE GITE MIDI\n"
        "VAISELLE MIDI\n"
        "NETTOYAGE GITE GOUTER\n"
        "PREPA REPAS SOIR\n"
        "NETTOYAGE GITE SOIR\n"
        "VAISSELLE SOIR\n"
        "COURSE"
...
if st.button(
    ...
    taches_par_enfant = {
        e: set() for e in prenoms
    }

    nb_taches_enfant = {
        e: 0 for e in prenoms
    }
    ...
    for jour in range(1, nb_jours + 1):
        ...
        for tache in taches:
            ...
            eligibles.sort(
                key=lambda e: nb_taches_enfant[e]
            )
            ...
            if len(assignes) < besoin:
                alertes.append(
                    f"Jour {jour} - {tache} : "
                    f"{besoin - len(assignes)} place(s) manquante(s)"
                )
